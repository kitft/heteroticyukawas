...
HYMfind_LR = float(sys.argv[3])
HYMfind_width = int(sys.argv[4])
HYMfind_depth = int(sys.argv[5])

vH_LR = float(sys.argv[6])
vH_width = int(sys.argv[7])
vH_depth = int(sys.argv[8])

print("HYM LR AND DEPTH AND WIDTH: ", HYMfind_LR, " and ", HYMfind_depth, "and",  HYMfind_width)
print("LR AND DEPTH AND WIDTH: ", vH_LR, " and ", vH_depth, "and",  vH_width)
...
if __name__ ==  '__main__':
    ...
    if train_02m20:
        #betamodel_02m20,training_historyBeta_02m20, measure = train_and_save_nn_HYM(free_coefficient,linebundleforHYM_02m20,depthBeta,widthBeta,nEpochsBeta,stddev=0.05,bSizes=[64],lRate=lRateBetha,alpha=alphabeta,load_network=False)
        start_time = time.time()
        betamodel_02m20,training_historyBeta_02m20, measure1 = train_and_save_nn_HYM(free_coefficient,linebundleforHYM_02m20,HYMfind_depth,HYMfind_width,nEpochsBeta,stddev=0.05,bSizes=[64],lRate=HYMfind_LR,alpha=alphabeta,load_network=False)
        end_time= time.time()
    else: 
    ...
    print("HYM: DEPTH, WIDTH, LR, time taken: ", HYMfind_depth, HYMfind_width, HYMfind_LR,measure1 , deltaT)
    ...
    if train_110m2:
        betamodel_110m2,training_historyBeta_110m2, measure2 = train_and_save_nn_HYM(free_coefficient,linebundleforHYM_110m2,depthBeta,widthBeta,nEpochsBeta,stddev=0.05,bSizes=[64],lRate=lRateBeta,alpha=alphabeta,load_network=False)
    else:
        betamodel_110m2,training_historyBeta_110m2=load_nn_HYM(free_coefficient,linebundleforHYM_110m2,depthBeta,widthBeta,nEpochsBeta,[64],set_weights_to_zero=return_zero_HYM,skip_measures=skip_measuresBeta)
    print("HYM: DEPTH, WIDTH, LR, time taken: ", HYMfind_depth, HYMfind_width, HYMfind_LR,measure2 , deltaT)
    #mem= = tracker.SummaryTracker()
    #print(sorted(mem(.create_summary(), reverse=True, key=itemgetter(2))[:10])
    delete_all_dicts_except('dataEval')
    gc.collect()
    ...
    if train_m1m322:
        betamodel_m1m322,training_historyBeta_m1m322, measure3 = train_and_save_nn_HYM(free_coefficient,linebundleforHYM_m1m322,depthBeta,widthBeta,nEpochsBeta,stddev=0.05,bSizes=[64],lRate=lRateBeta,alpha=alphabeta,load_network=False)
    else:
        betamodel_m1m322,training_historyBeta_m1m322=load_nn_HYM(free_coefficient,linebundleforHYM_m1m322,depthBeta,widthBeta,nEpochsBeta,[64],set_weights_to_zero=return_zero_HYM,skip_measures=skip_measuresBeta)



    print("HYM: DEPTH, WIDTH, LR, time taken: ", HYMfind_depth, HYMfind_width, HYMfind_LR,measure3 , deltaT,"\n")
    data_to_append = [HYMfind_depth, HYMfind_width, HYMfind_LR, deltaT, measure1,measure2, measure3]

    # Open the file in append mode and write the data
    with open('HYM_finding_archs_all_HYM_m1.txt', 'a', newline='') as file:
    ...
    if train_vH:
        ...
        start_time = time.time()
        HFmodel_vH,trainingHistoryHF_vH,measure_how_well_rounded=train_and_save_nn_HF(free_coefficient,linebundleforHYM_02m20,betamodel_02m20,functionforbaseharmonicform_jbar_for_vH,vH_depth,vH_width,nEpochsSigma,[64],lRate=vH_LR,alpha=alphasigma1,stddev=stddev_H33,final_layer_scale=final_layer_scale_H33)
        #was /100
        end_time = time.time()
    else:
    ...
    deltaT = end_time - start_time
    print("HF: DEPTH, WIDTH, LR, time taken: ", vH_depth, vH_width, vH_LR,measure_how_well_rounded, deltaT)
    data_to_append = [vH_depth, vH_width, vH_LR, measure_how_well_rounded,deltaT]

    # Open the file in append mode and write the data
    with open('finding_archs_m1.txt', 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(data_to_append)
    exit()
    delete_all_dicts_except('dataEval')
    ...
    if train_vQ3:
        HFmodel_vQ3,trainingHistoryHF_vQ3, measure = train_and_save_nn_HF(free_coefficient,linebundleforHYM_110m2,betamodel_110m2,functionforbaseharmonicform_jbar_for_vQ3,depthSigma,widthSigma,nEpochsSigma,[64],lRate=lRateSigma,alpha=alphasigma1,stddev=stddev_H33,final_layer_scale=final_layer_scale_H33)
    else:
        HFmodel_vQ3,trainingHistoryHF_vQ3=load_nn_HF(free_coefficient,linebundleforHYM_110m2,betamodel_110m2,functionforbaseharmonicform_jbar_for_vQ3,depthSigma,widthSigma,nEpochsSigma,[64],alpha=alphasigma1,skip_measures=skip_measuresHF)
    delete_all_dicts_except('dataEval')
    gc.collect()
    tf.keras.backend.clear_session()
    ...
    #print(sorted(mem(.create_summary(), reverse=True, key=itemgetter(2))[:10])
    if train_vU3:
        HFmodel_vU3,trainingHistoryHF_vU3, measure = train_and_save_nn_HF(free_coefficient,linebundleforHYM_110m2,betamodel_110m2,functionforbaseharmonicform_jbar_for_vU3,depthSigma,widthSigma,nEpochsSigma,[64],lRate=lRateSigma,alpha=alphasigma1,stddev=stddev_H33,final_layer_scale=final_layer_scale_H33)
    else:
        HFmodel_vU3,trainingHistoryHF_vU3=load_nn_HF(free_coefficient,linebundleforHYM_110m2,betamodel_110m2,functionforbaseharmonicform_jbar_for_vU3,depthSigma,widthSigma,nEpochsSigma,[64],alpha=alphasigma1,skip_measures=skip_measuresHF)
    ...
    if train_vQ1:
        HFmodel_vQ1,trainingHistoryHF_vQ1, measure = train_and_save_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,functionforbaseharmonicform_jbar_for_vQ1,depthSigma2,widthSigma2,nEpochsSigma2,[64],lRate=lRateSigma2,alpha=alphasigma2,stddev=stddev_Q12U12,final_layer_scale=final_layer_scale_Q12U12)
    else:
        HFmodel_vQ1,trainingHistoryHF_vQ1=load_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,functionforbaseharmonicform_jbar_for_vQ1,depthSigma2,widthSigma2,nEpochsSigma2,[64],alpha=alphasigma2,skip_measures=skip_measuresHF)
    ...
    if train_vQ2:   
        HFmodel_vQ2,trainingHistoryHF_vQ2, measure = train_and_save_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,functionforbaseharmonicform_jbar_for_vQ2,depthSigma2,widthSigma2,nEpochsSigma2,[64],lRate=lRateSigma2,alpha=alphasigma2,stddev=stddev_Q12U12,final_layer_scale=final_layer_scale_Q12U12)
    else:
        HFmodel_vQ2,trainingHistoryHF_vQ2=load_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,functionforbaseharmonicform_jbar_for_vQ2,depthSigma2,widthSigma2,nEpochsSigma2,[64],alpha=alphasigma2,skip_measures=skip_measuresHF)
    delete_all_dicts_except('dataEval')
    gc.collect()
    tf.keras.backend.clear_session()
    ...
    if train_vU1:
        HFmodel_vU1,trainingHistoryHF_vU1, measure = train_and_save_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,functionforbaseharmonicform_jbar_for_vU1,depthSigma2,widthSigma2,nEpochsSigma2,[64],lRate=lRateSigma2,alpha=alphasigma2,stddev=stddev_Q12U12,final_layer_scale=final_layer_scale_Q12U12)
    else:
        HFmodel_vU1,trainingHistoryHF_vU1=load_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,functionforbaseharmonicform_jbar_for_vU1,depthSigma2,widthSigma2,nEpochsSigma2,[64],alpha=alphasigma2,skip_measures=skip_measuresHF)
    delete_all_dicts_except('dataEval')
    gc.collect()
    tf.keras.backend.clear_session()
    ...
    if train_vU2:
        HFmodel_vU2,trainingHistoryHF_vU2, measure = train_and_save_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,functionforbaseharmonicform_jbar_for_vU2,depthSigma2,widthSigma2,nEpochsSigma2,[64],lRate=lRateSigma2,alpha=alphasigma2,stddev=stddev_Q12U12,final_layer_scale=final_layer_scale_Q12U12)
    else:
        HFmodel_vU2,trainingHistoryHF_vU2=load_nn_HF(free_coefficient,linebundleforHYM_m1m322,betamodel_m1m322,functionforbaseharmonicform_jbar_for_vU2,depthSigma2,widthSigma2,nEpochsSigma2,[64],alpha=alphasigma2,skip_measures=skip_measuresHF)
    delete_all_dicts_except('dataEval')
    gc.collect()
    tf.keras.backend.clear_session()
    ...
    for use_trained in [True,False]:
        ...
        if use_trained:
            ...
            hvQ3b = tf.einsum('x,xb->xb', tf.cast(betamodel_110m2(real_pts), tf.complex64), tf.math.conj(vQ3))
            ...
            hvU3b = tf.einsum('x,xb->xb', tf.cast(betamodel_110m2(real_pts), tf.complex64), tf.math.conj(vU3))
            ...
            hvQ1b = tf.einsum('x,xb->xb', tf.cast(betamodel_m1m322(real_pts), tf.complex64), tf.math.conj(vQ1))
            ...
            hvQ2b = tf.einsum('x,xb->xb', tf.cast(betamodel_m1m322(real_pts), tf.complex64), tf.math.conj(vQ2))
            ...
            hvU1b = tf.einsum('x,xb->xb', tf.cast(betamodel_m1m322(real_pts), tf.complex64), tf.math.conj(vU1))
            ...
            hvU2b = tf.einsum('x,xb->xb', tf.cast(betamodel_m1m322(real_pts), tf.complex64), tf.math.conj(vU2))
